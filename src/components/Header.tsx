...
export function Header() {
  ...
  return (
    <div className="fixed inset-x-0 top-0 z-50 flex flex-col">
      {/* TOP INFORMATION BAR */}
      <div className="bg-[#071A35]/90 border-b border-white/5 text-white/80 text-xs py-2 px-6 backdrop-blur-md hidden md:block">
        <div className="max-w-[1400px] mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-6">
            ...
            <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="hover:text-[#00AEEF] transition-colors">
              <IconBrandLinkedin className="h-4 w-4" />
            </a>
            <a href="https://wa.me/918130720777" target="_blank" rel="noopener noreferrer" className="hover:text-[#00AEEF] transition-colors">
      ...
      <header
        className={cn(
          "w-full transition-all duration-300 border-b bg-white border-slate-200 shadow-sm",
          scrolled ? "py-3" : "py-4"
        )}
      ...
      >
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 flex items-center justify-between gap-4 sm:gap-8 relative">
            ...
            {navList.map((item) => {
              if ("to" in item) {
                ...
                return (
                  <Link
                    ...
                    href={item.to}
                    className={cn(
                      "px-3 py-2 text-sm font-medium transition-all relative group/link",
                      isActive ? "text-[#0A4ABF]" : "text-[#071A35]/80 hover:text-[#0A4ABF]"
                    )}
                  >
                    {item.label}
                    <span className={cn(
              ...
              return (
                <div key={item.label} onMouseEnter={() => setOpen(item.label)}>
                  <button
                    className={cn(
                      "px-3 py-2 text-sm font-medium flex items-center gap-1 transition-all rounded-sm",
                      isOpen || hasActiveChild ? "text-[#0A4ABF]" : "text-[#071A35]/80 hover:text-[#0A4ABF]"
                    )}
                  >
                    {item.label}
                  ...
                  <AnimatePresence>
                    {isOpen && (
                      <motion.div
                        ...
                        className="absolute left-1/2 top-full mt-2 w-[calc(100vw-3rem)] max-w-[850px] bg-white text-[#071A35] rounded-sm shadow-2xl border border-slate-200 overflow-hidden z-50 grid grid-cols-12"
                      >
                        {/* Mega Menu Columns */}
                        <div className="col-span-8 p-8 grid grid-cols-2 gap-8 border-r border-slate-100">
                          {item.mega.map((col) => (
                            <div key={col.col} className="space-y-4">
                              <div className="text-xs uppercase tracking-wider text-slate-400 font-bold border-b border-slate-100 pb-2">
                                {col.col}
                              </div>
                                ...
                                {col.items.map((mi) => (
                                  <li key={mi.to}>
                                    <Link
                                      ...
                                      className="group/item flex gap-3 rounded-sm p-1.5 hover:bg-slate-50 transition-colors"
                                    >
                                      <div className="h-8 w-8 rounded-sm bg-[#00AEEF]/10 grid place-items-center text-[#0A4ABF] shrink-0 mt-0.5">
                                      ...
                                      <div className="leading-tight">
                                        <div className="font-semibold text-sm text-[#071A35] group-hover/item:text-[#0A4ABF] transition-colors flex items-center gap-1">
                                          {mi.label}
                                          <IconArrowUpRight className="h-3 w-3 opacity-0 group-hover/item:opacity-100 transition-opacity" />
                                        </div>
                        ...
                        <div className="col-span-4 bg-slate-50 p-8 flex flex-col justify-between">
                          <div>
                            <span className="text-[10px] uppercase tracking-wider font-extrabold text-[#00AEEF] block mb-2">Featured Showcase</span>
                            <h4 className="font-bold text-sm text-[#071A35]">{item.featured.title}</h4>
                          ...
                          <Link
                            ...
                            onClick={() => setOpen(null)}
                            className="inline-flex items-center justify-between bg-[#071A35] hover:bg-[#0A4ABF] text-white font-bold text-xs uppercase tracking-wider p-3 rounded-sm transition-colors mt-6"
                          >
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <a href="tel:+918130720777" className="hidden xl:inline-flex h-10 w-10 items-center justify-center border border-slate-200 rounded-sm hover:bg-slate-50 transition-all text-[#071A35]">
              <IconPhone className="h-4 w-4 text-[#0A4ABF]" />
            </a>
            <a href="mailto:support@smsmengineers.in" className="hidden xl:inline-flex h-10 w-10 items-center justify-center border border-slate-200 rounded-sm hover:bg-slate-50 transition-all text-[#071A35]">
              <IconMail className="h-4 w-4 text-[#0A4ABF]" />
            </a>
            <Link
              href="/query-form"
              className="hidden sm:inline-flex items-center gap-2 bg-[#00AEEF] text-[#071A35] font-extrabold text-xs uppercase tracking-wider px-5 py-3 rounded-sm hover:bg-white hover:shadow-[0_0_15px_rgba(255,255,255,0.4)] transition-all"
            >
              Request Quote <IconArrowUpRight className="h-4 w-4" />
            </Link>
            <button
              className="lg:hidden text-[#071A35] p-2"
        ...
        <AnimatePresence>
          {mobileOpen && (
            <motion.div
              ...
              className="lg:hidden overflow-hidden bg-[#071A35] border-t border-white/10"
            >
              <div className="px-6 py-6 space-y-2 max-h-[80vh] overflow-y-auto">
                {navList.map((item) =>
                  "to" in item ? (
                    <Link
                      ...
                      className="block py-3 text-white/90 border-b border-white/5 font-medium hover:text-[#00AEEF]"
                    >
                      {item.label}
                    </Link>
                  ) : (
                    <div key={item.label} className="border-b border-white/5 py-3">
                      <div className="text-xs uppercase tracking-wider text-[#00AEEF] font-bold mb-2">
                        {item.label}
                      </div>
                ...
                <Link
                  href="/query-form"
                  onClick={() => setMobileOpen(false)}
                  className="mt-6 w-full justify-center inline-flex items-center gap-2 bg-[#00AEEF] text-[#071A35] font-bold py-3 rounded-sm"
                >
                  Request Quote <IconArrowUpRight className="h-4 w-4" />
                </Link>
              </div>
